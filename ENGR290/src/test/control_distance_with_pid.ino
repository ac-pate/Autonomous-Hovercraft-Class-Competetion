...
const float pidDistance[3] = { 3, 0.5, 0.5 };
...
void foward() {
  ...
  // get control values 
  float fan_speed = distanceControl(dt, distance, distanceSetpoint);
  float control_angle = centerAngle + update(dt, curr_angle, angleSetpoint);
...
float distanceControl(float dt, float currentValue, float targetValue) {
  ...
  float D = pidDistance[2] * deriveMeasure;

  float result = P + I + D;

  return constrain(result, 200, 255);
}
